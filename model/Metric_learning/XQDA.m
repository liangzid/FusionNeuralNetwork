...
%
% Input:
%   <galX>: features of gallery samples. Size: [n, d] %输入的检测样本的特征？？
%   <probX>: features of probe samples. Size: [m, d]  %概率样本的特征
%   <galLabels>: class labels of the gallery samples  %输入的检测样本的标签
%   <probLabels>: class labels of the probe samples   %输入的概率样本的标签
%   [options]: optional parameters. A structure containing any of the
%   following fields:
%       lambda: the regularizer. Default: 0.001       %正则项的系数
%       qdaDims: the number of dimensions to be preserved in the learned  %在学习中被保留的qda维度
%       subspace. Negative values indicate automatic dimension selection by
%       perserving latent values larger than 1. Default: -1.               %一个负值，它用来保证在保留潜在值的自动维度选择中潜在值大于1
%       verbose: whether to print the learning details. Default: false     %是否显示学习细节
%
% Output:
%   W: the subspace projection matrix. Size: [d, r], where r is the        %子空间矩阵，r是子空间的维数
%   subspace dimension.
%   M: the learned metric kernel. Size: [r,r]                              %学习度量的核
%   inCov: covariance matrix of the intra-personal difference class. Size: %类内数据的协方差矩阵
%   [r,r]
%   exCov: covriance matrix of the extra-personal difference class. Size:  %类间数据的协方差矩阵
%   [r,r]
%
